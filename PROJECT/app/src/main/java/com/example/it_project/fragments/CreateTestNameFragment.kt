...
class CreateTestNameFragment : AppCompatDialogFragment() {
    ...
    override fun onResume() {
        ...
        button_commit_create_test_name.setOnClickListener {
            ...
            if((!edit_text_test_name.text.isEmpty())&&(!edit_text_time.text.isEmpty())){
                val listener = object : ValueEventListener {
                    override fun onDataChange(snapshot: DataSnapshot) {
                        var listTests = ArrayList<String>()
                        for (privateTest in snapshot.child("private tests").children) {
                            listTests.add(
                                privateTest.child("test name").getValue(String::class.java)!!
                            )
                        }
                        for (publicTest in snapshot.child("public test").children) {
                            listTests.add(
                                publicTest.child("test name").getValue(String::class.java)!!
                            )
                        }
                        if (!listTests.contains(CURRENT_TEST_NAME)) {
                            ...
                            intentCreateTest.putExtra(
                                "TestName",
                                edit_text_test_name.text.toString().trim()
                            )
                            startActivity(intentCreateTest)
                            fragmentManager?.beginTransaction()?.remove(this@CreateTestNameFragment)
                                ?.commit()
                            ...
                            Toast.makeText(
                                context,
                                "Уже есть тест с таким названием",
                                Toast.LENGTH_SHORT
                            ).show()
                        }
                    }
